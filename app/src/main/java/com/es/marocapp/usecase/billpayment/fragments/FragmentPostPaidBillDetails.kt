    ...
    override fun init(savedInstanceState: Bundle?) {
        ...
        if (mActivityViewModel.isBillUseCaseSelected.get()!!) {
            for (i in mActivityViewModel.PostPaidFinancialResourceInfoObserver.get()!!.invoices.indices) {
                var item =
                    mActivityViewModel.PostPaidFinancialResourceInfoObserver.get()!!.invoices[i]
                listOfCustomInvoice.add(
                    InvoiceCustomModel(
                        false,
                        item.month,
                        item.ohrefnum,
        ...
        if (mActivityViewModel.isFatoratiUseCaseSelected.get()!!) {
            ...
            if (mActivityViewModel.selectedCreancer.get()
                    .equals("Autoroutes du Maroc") || mActivityViewModel.selectedCreancer.get()
                    ?.contains("TGR")!!
            ) {
            ...
            for (i in mActivityViewModel.fatoratiStepFourObserver.get()!!.params.indices) {
                ...
                if (!item.idArticle.contains("FRAIS")) {
                    listOfFatoratiCustomInvoice.add(
                        FatoratiCustomParamModel(
                            false,
                            item.description,
                            item.idArticle,
                            item.prixTTC,
                            item.typeArticle,
                            mActivityViewModel.showAutoDuMorocViews
                        )
                    )
                    listOfFatoratiCustomDateInvoice.add(
                        FatoratiCustomDateParamModel(
                            false,
                            item.description,
                            item.idArticle,
                            item.prixTTC,
                            item.typeArticle,
                            getDateFromString(item.description)
                        )
                    )
                }
            }
        ...
        mDataBinding.selectAllCheckBox.setOnCheckedChangeListener(object :
    ...
    private fun setStrings() {
        ...
        if (mActivityViewModel.showAutoDuMorocViews) {
            if (mActivityViewModel.selectedCreancer.get()?.contains("TGR") == true) {
                mDataBinding.selectInvocieLabel.text =
                    LanguageData.getStringValue("BillPaymentSelectInvoicesToPayTGR")
            } else {
                mDataBinding.selectInvocieLabel.text =
                    LanguageData.getStringValue("BillPaymentSelectInvoicesToPayAutoDM")
            }
        } else {
            mDataBinding.selectInvocieLabel.text =
                LanguageData.getStringValue("BillPaymentSelectInvoicesToPay")
        }
        ...
        if (selectedFatoratiListOfInvoice.size.equals(0)) {
            ...
            for (i in selectedFatoratiListOfInvoice.indices) {
                ...
                mActivityViewModel.totalSelectedBillAmount =
                    ((mActivityViewModel.totalSelectedBillAmount.toDouble() + convertedBillAmount.toDouble())).toString()
            ...
            if (!mActivityViewModel.fatoratiStepFourObserver.get()?.typeFrais.isNullOrEmpty()) {
                if (!mActivityViewModel.fatoratiStepFourObserver.get()?.valeurFrais.isNullOrEmpty()) {
                    if (mActivityViewModel.fatoratiStepFourObserver.get()?.typeFrais.equals(
                            Constants.BILL_PAYMENT_TYPE_FORFAIT_FACTURE
                        )
                    ) {
                        var forfaitFactureFeeToApplied =
                            mActivityViewModel.fatoratiStepFourObserver.get()?.valeurFrais?.toInt()
                        var fortaitFactureFee =
                            selectedListOfInvoice.size * forfaitFactureFeeToApplied!!
                        var fortaitFactureFeeInDouble =
                            Constants.converValueToTwoDecimalPlace(fortaitFactureFee.toDouble())
                        var totalFeeAfterAddingForfaitFactureFee =
                            mActivityViewModel.totalSelectedBillAmount.toDouble() + fortaitFactureFee

                        Logger.debugLog(
                            "forfaitFactureFeeToApplied",
                            forfaitFactureFeeToApplied.toString()
                        )
                        Logger.debugLog(
                            "fortaitFactureFee",
                            fortaitFactureFee.toString() + "Number Of Bill Selected = ${selectedListOfInvoice.size}"
                        )
                        Logger.debugLog("fortaitFactureFeeInDouble", fortaitFactureFeeInDouble)
                        Logger.debugLog(
                            "fortaitFactureFeeAfterAddingInTotal",
                            totalFeeAfterAddingForfaitFactureFee.toString() + "Total Bill Amount is = ${mActivityViewModel.totalSelectedBillAmount}"
                        )
                    ...
                    } else if (mActivityViewModel.fatoratiStepFourObserver.get()?.typeFrais.equals(
                            Constants.BILL_PAYMENT_TYPE_FORFAIT
                        )
                    ) {
                        var fortaitFeeToApplied =
                            mActivityViewModel.fatoratiStepFourObserver.get()?.valeurFrais?.toDouble()
                        var forfaitFeeCalculated =
                            mActivityViewModel.totalSelectedBillAmount.toDouble() + fortaitFeeToApplied!!
                        ...
                        mActivityViewModel.fatoratiFeeAmountCaseImplemented = true
                            ...
                            Constants.BILL_PAYMENT_TYPE_COMISSION
                        )
                    ) {
                        var commissionFee =
                            mActivityViewModel.fatoratiStepFourObserver.get()?.valeurFrais?.toDouble()
                        var commissionFeeCalculated: Double =
                            ((mActivityViewModel.totalSelectedBillAmount.toDouble() * commissionFee!!) / 100)
                        commissionFeeCalculated =
                            Constants.converValueToTwoDecimalPlace(commissionFeeCalculated)
                                .toDouble()
                        var commissionFeeCalculatedAddedInTotalAmount =
                            commissionFeeCalculated.toDouble() + mActivityViewModel.totalSelectedBillAmount.toDouble()
                        ...
                        Logger.debugLog(
                            "commissionFeeCalculatedAddedInTotalAmount",
                            commissionFeeCalculatedAddedInTotalAmount.toString()
                        )
            ...
            mActivityViewModel.requestForFatoratiQuoteApi(
                activity,
                amountToSendInRequest,
                mActivityViewModel.fatoratiStepFourObserver.get()?.refTxFatourati.toString(),
                mActivityViewModel.fatoratiStepFourObserver.get()?.totalAmount.toString(),
    ...
    private fun payPostPaidBillsCall() {
        if (selectedListOfInvoice.size.equals(0)) {
            ...
            for (i in selectedListOfInvoice.indices) {
                ...
                var convertedBillAmount =
                    (selectedListOfInvoice[i].openAmount.toDouble() / Constants.AMOUNT_CONVERSION_VALUE.toDouble()).toString()
                mActivityViewModel.listOfSelectedBillAmount.add(convertedBillAmount)
                mActivityViewModel.totalSelectedBillAmount =
                    ((mActivityViewModel.totalSelectedBillAmount.toDouble() + convertedBillAmount.toDouble())).toString()
                Log.i("TotalBillAmount", mActivityViewModel.totalSelectedBillAmount)
                ...
                mActivityViewModel.requestForPostPaidBillPaymentQuoteApi(
                    activity,

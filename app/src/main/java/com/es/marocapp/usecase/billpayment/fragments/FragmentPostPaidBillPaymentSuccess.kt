...
class FragmentPostPaidBillPaymentSuccess : BaseFragment<FragmentBillPaymentSuccessBinding>(),
    ...
    override fun init(savedInstanceState: Bundle?) {
        ...
        mDataBinding.addToFavoriteCheckBox.setOnClickListener {
            DialogUtils.showAddToFavoriteDialoge(activity as BillPaymentActivity,
...
//                                mActivityViewModel.fatoratiStepThreeObserver.get()!!.refTxFatourati
                            ...
                            var fatoratiNickName =
                                "Util_${mActivityViewModel.fatoratiTypeSelected.get()!!.nomCreancier}@$nickName" +
                                        ",${mActivityViewModel.fatoratiTypeSelected.get()!!.codeCreance}," +
                                        mActivityViewModel.fatoratiTypeSelected.get()!!.codeCreancier + "," + mActivityViewModel.validatedParams + "," +
                                        mActivityViewModel.fatoratiStepThreeObserver.get()!!.refTxFatourati

                            mActivityViewModel.requestForAddFavoritesApi(
                                activity,
                                fatoratiNickName,
                            ...
                            if (mActivityViewModel.isInternetSelected.get()!!) {
                                ...
                                billPaymentNumber =
                                    Constants.getPostPaidInternetDomainAlias(mActivityViewModel.transferdAmountTo)
                            } else if (mActivityViewModel.isPostPaidMobileSelected.get()!!) {
                                billPaymentNickName =
                                    "Telec_PostpaidMobile@$nickName,${mActivityViewModel.mCodeEntered}"
                                billPaymentNumber =
                                    Constants.getPostPaidMobileDomainAlias(mActivityViewModel.transferdAmountTo)
                            } else if (mActivityViewModel.isPostPaidFixSelected.get()!!) {
                                billPaymentNickName =
                                    "Telec_PostpaidFix@$nickName,${mActivityViewModel.mCodeEntered}"
                                billPaymentNumber =
                                    Constants.getPostPaidFixedDomainAlias(mActivityViewModel.transferdAmountTo)
                            }
                            mActivityViewModel.requestForAddFavoritesApi(
                                activity,
                                billPaymentNickName,
        ...
        if (mActivityViewModel.isFatoratiUseCaseSelected.get()!!) {
            ...
            mDataBinding.tvFatoratiFeeTitle.text =
                LanguageData.getStringValue("FatouratiTotalFeeCharged")
    ...
    private fun populateFatoratiBillList() {
        ...
        mFatoratiBillPaidAdapter = FatoratiPaidBillAdapter(
            mActivityViewModel.getPostPaidFatoratiResponseListner.value!!,
            mActivityViewModel.listOfSelectedBillAmount,
            mActivityViewModel.fatoratiFee,
            receiverNumber
        )
    ...
    private fun populatePaidBillList() {
        var receiverNumber = mActivityViewModel.transferdAmountTo.substringBefore("@")
        receiverNumber = receiverNumber.substringBefore("/")
        mBillPaidAdapter = PaidBillStatusAdpater(
            mActivityViewModel.listOfPostPaidBillPayment,
            mActivityViewModel.listOfSelectedBillAmount,
            mActivityViewModel.listOfSelectedBillFee,
            receiverNumber,
            mActivityViewModel.ReceiverName
        )
